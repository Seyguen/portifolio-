var enviar = document.querySelector(".formcontato");
function validaForm(event) {
  ...
  if (nome.value.length == 0) {
    ...
    event.preventDefault(); //exibe mensagem que o campo deve ser preenchido
  ...
  if (nome.value.length > 50) {
    alert(
      "O campo de nome deve conter no máximo 50 caracteres, por favor tente novamente!"
    );
  ...
  if (assunto.value.length > 50) {
    alert(
      "O campo assunto deve conter no máximo 50 caracteres, por favor tente novamente!"
    );
    event.preventDefault();
    assunto.value = "";
    assunto.focus();
  ...
  if (mensagem.value.length > 300) {
    alert(
      "O campo mensagem deve conter no máximo 300 caracteres, por favor tente novamente!"
    );
    event.preventDefault();
    mensagem.value = "";
    mensagem.focus();
...
//exiibe mensagem para preechimento do campo vazio
...
for (var i = 0; i < elementos.length; i++) {
  elementos[i].addEventListener("change", function () {    
    if (
      ...
      enviar.style.background = "#D4E4FA";
